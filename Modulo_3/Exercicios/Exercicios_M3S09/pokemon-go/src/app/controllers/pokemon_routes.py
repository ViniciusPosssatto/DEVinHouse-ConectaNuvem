...
@pokemons.route("/list_pokemon_lower_HP", methods = ["GET"])
def list_pokemon_lower_HP():
  pokemons_query = mongo_client.pokemons.find().sort("HP", 1)
...
@pokemons.route("/list_pokemon_by_lower_winners/<int:page>/<int:limit>", methods = ["GET"])
def list_pokemon_by_lower_winners(page, limit):
  pokemons_query = mongo_client.pokemons.aggregate([
    {
        '$lookup': {
            'from': 'combats', 
